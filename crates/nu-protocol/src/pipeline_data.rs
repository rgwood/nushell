...
impl PipelineData {
    ...
    pub fn follow_cell_path(
        ...
        for path_member in cell_path {
            ret = match ret {
                PipelineData::Value(v, _) => PipelineData::Value(
                    v.follow_single_path_member(&path_member, insensitive)?,
                    None,
                ),
                PipelineData::ListStream(mut stream, _) => {
                    match path_member {
                        ...
                        PathMember::Int { val, .. } => match stream.nth(val) {
                ...
                PipelineData::ExternalStream { .. } => {
                    return Err(ShellError::IOError(
                        "can't follow cell paths into external streams".into(),
                    ))
                }
                PipelineData::Empty => {
                    return Err(ShellError::IOError(
                        "can't follow cell paths into an empty pipeline".into(),
                    ))
                }
            };
        }
